...
class MensajeModel extends Model{
    ...
    public function save($mensaje){
        if(!$mensaje->getFechaEnviado() == 'CURRENT_TIMESTAMP()'){
          $mensaje->setFechaEnviado('CURRENT_TIMESTAMP()');
        }
        $query="INSERT INTO Mensaje (asunto,contenido,fecha_enviado, fecha_visto, id_emisor, id_receptor) VALUES('".trim($mensaje->getAsunto())."','".trim($mensaje->getContenido())."',".$mensaje->getFechaEnviado().",'".$mensaje->getFechaVisto()."',".trim($mensaje->getIdEmisor()).",".trim($mensaje->getIdReceptor()).");";
        error_log("---------------------------------");
        error_log($query);
